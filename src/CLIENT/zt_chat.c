...
int chat_join(int socket, const user_t *user)
{
	...
	if (socket < 0 || user == NULL) {
		printf("[chat_join] Invalid argument\n");
		return -1;
	}

	snprintf(json_body, sizeof(json_body),
		"{\"user_id\": \"%s\", \"name\": \"%s\", \"password\": \"%s\", \"mdn\": \"%s\"}",
		user->user_id, user->name, user->password, user->mdn);
		     ...
		     const char *room_type, const char *members)
{
	...
	if (socket < 0 || room_id == NULL || password == NULL || user_id == NULL ||
	    room_type == NULL || members == NULL) {
		printf("[chat_create_room] Invalid argument\n");
		return -1;
	}

	snprintf(json_body, sizeof(json_body),
		"{\"room_id\": \"%s\", \"password\": \"%s\", \"user_id\": \"%s\", "
		"\"room_type\": \"%s\", \"members\": \"%s\"}",
		room_id, password, user_id, room_type, members);
...
int chat_list_friends(int socket, const char *user_id)
{
	...
	if (socket < 0 || user_id == NULL || user_id[0] == '\0') {
		printf("[chat_list_friends] Invalid argument\n");
		return -1;
	}

	snprintf(path, sizeof(path), "/friends?user_id=%s", user_id);
	return SOCKET_SendHttpRequest(socket, "localhost", "GET", path);
}
...
int chat_invite_room(int socket, const char *room_id, const char *user_id, const char *members)
{
	...
	if (socket < 0 || room_id == NULL || user_id == NULL || members == NULL) {
		printf("[chat_invite_room] Invalid argument\n");
		return -1;
	}

	snprintf(json_body, sizeof(json_body),
		"{\"room_id\": \"%s\", \"user_id\": \"%s\", \"members\": \"%s\"}",
		room_id, user_id, members);

	len = snprintf(req_buf, sizeof(req_buf), HTTP_REQUEST_FMT,
		"POST", "/room/invite", "localhost:8080", strlen(json_body), json_body);

	if (len <= 0 || (size_t)len >= sizeof(req_buf)) {
		printf("[chat_invite_room] Request build fail\n");
		return -1;
	}
